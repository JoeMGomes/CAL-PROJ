...
void updateColors(nodeEdge_t graph, int color)
{
    ...
    for (long unsigned int i = 1; i < graph.points.size(); i++)
    {
        if (graph.points[i]->getType() == DELIVERY)
            ...
            gv->setVertexColor(graph.points[i]->getID(), cor);
    }
...
{
    gv->setVertexColor(centralPoint->getID(), RED);
...
bool checkValidPoints(int p1, int p2)
{
    ...
    if (getPath(p1).lenght == 0)
        return false;
    dijkstra(p1, p2);
    if (getPath(p2).lenght == 0)
        return false;
    return true;
}
...
void AdicionaEncomenda(int source, int delivery)
{
    ...
    if (Source != nullptr && Delivery != nullptr && checkValidPoints(source, delivery))
    {
        ...
        cout << "Your order hasn't been added." << endl
             << "Please check if the points ID's are correct and if the points are connected." << endl;
}
...
std::vector<nodeEdge_t *> nearestNeighbour(std::vector<Package> packages, Point *supportPoint, int color)
{
    ...
    while (pointsToGo.size() != 0)
    {
        ...
        if (pointsToGo.at(temp)->getType() == SOURCE)
        { //Se o ponto era uma Source, coloca a delivery
            pointsToGo.at(temp) = packages.at(temp).getDeliveryPoint();
        }
        else if (pointsToGo.at(temp)->getType() == DELIVERY)
        { //se era uma delivery retira do vetor
...
void menuControler()
{
    ...
    case 2:
    {
        ...
        int id;
        cin >> id;
        gv->setVertexColor(centralPoint->getID(),BLACK);
